...
public class APIController {
	...
	public ResponseEntity<?> updateAccount(ModelMap model, @PathVariable("userid") Long userId,
			@Validated @RequestBody inputUpdateUser inputUser, BindingResult errors) {

		try {
			if (errors.hasErrors()) {
	            Map<String, String> errorsMap = new HashMap<>();
	            errors.getFieldErrors().forEach(err -> {
	            	String errorField = "err_"+err.getField();
	            	errorsMap.put(errorField, err.getDefaultMessage());
	            });
			...
			if (!user.getUserName().equals(inputUser.getUserName())
					...
					|| !user.getEmail().equals(inputUser.getEmail())
							&& userserv.existsByEmail(inputUser.getEmail())) {
